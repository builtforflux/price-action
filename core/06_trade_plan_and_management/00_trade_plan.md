...
- Entry、execution / order lifecycle、planned protective stop、target、outcome criterion、概率和仓位必须来自同一版本；成交后的 active protective stop 还必须与实际 fill 和受保护数量一致。不能拼接不同方案中最有利的输入；若另有 catastrophe backup，也必须计入最坏损失与仓位约束。
