...
首次学习时，进入本目录前应已按编号完整读完 `core/00_method` 至 `core/04_patterns`；进入后按 `00`–`05` 顺序读完本目录全部正文。下列家族用于比较不同命题，不是选读菜单。
...
Second entry 和 trap 是触发顺序与失败机制，不是独立 Setup 家族；其定义见[二次入场和陷阱](../03_acceptance_and_order_logic/03_second_entries_and_traps.md)。
...
| 逆势修正 scalp | 第一次逆势反转按 minor 处理，逆势方做短线 | 两腿修正或先进入 trading range；期待 MTR 仅限预写 runner，否则退出后按新 MTR 计划重新入场 |
...
6. [`05_minor_reversal_scalp.md`](05_minor_reversal_scalp.md)：第一次逆势反转的短线命题及其与 MTR 的边界。
