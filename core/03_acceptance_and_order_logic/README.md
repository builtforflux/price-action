...
本目录界定 stop entry、protective stop 和 limit-order behavior，判断一次尝试是否获得接受，并区分 second signal / second entry、trapped in / trapped out、entry disappointment、premise 变化，以及 Brooks success / failure 与适用 objective。
...
2. [`01_acceptance_and_failure.md`](01_acceptance_and_failure.md)：follow-through、失望、Brooks success / failure 与适用 objective、scalper's profit 结果边界、trapped in / trapped out，以及 Pain Trade 行为路径模型。
3. [`02_limit_order_market.md`](02_limit_order_market.md)：双边限价交易占优时的行为，以及 touch / cross、partial / actual fill 和风险边界。
