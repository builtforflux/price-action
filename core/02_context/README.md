# Context 专题导航：背景、位置和控制权

> **状态：Core / Index**
>
> 本页说明 Context 专题的职责和文件入口，不定义订单触发或完整交易计划。

## 职责

本目录把 Market Cycle 放回具体位置、控制权、周期和时段中，并判断前方目标空间。它回答一个候选为何在这里可能有意义，以及哪一方更可能控制。

订单是否触发、结果是否被接受属于[接受、失败与订单逻辑](../03_acceptance_and_order_logic/README.md)；如何形成入场命题属于 [Setups](../05_setups/README.md)。

## 文件

1. [`00_context_location_control.md`](00_context_location_control.md)：context、location、buying / selling pressure、control 与 Always In 的总体框架。
2. [`01_support_resistance_targets.md`](01_support_resistance_targets.md)：支撑阻力区域、test / confluence、magnets 和目标构造。
3. [`02_time_and_timeframes.md`](02_time_and_timeframes.md)：周期、session state、最终日型、开盘对象分层和尾段时间风险。

概念总图和权威定义归属见[核心框架](../README.md)。
