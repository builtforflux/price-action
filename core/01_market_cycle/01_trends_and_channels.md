...
在连续谱上，形成方向控制的 breakout phase（spike）几乎没有 pullback；紧通道的回调通常只有 1—3 根，重叠少，常见缺口、微缺口或近似缺口；宽通道的回调通常更深、更久，常穿过前一突破点，甚至超过前一腿的 50%，使双边都能走出可交易的腿；trading range 中多空接近平衡。这里的根数、深度和重叠只用于识别典型结构，不是机械阈值。
...
课程在具有清楚倾斜趋势属性的通道语境反复给出约 `75/25` 的长期演化先验：多数通道最终先发生反向趋势线突破并转向交易区间，少数出现强同向突破。通道越平、越接近 trading range，这组先验越应向 `50/50` 靠近。这个 `75%` 不表示直接形成相反趋势，也不等于每一个反向 signal 都有 `75%` 胜率。
...
课程也把宽通道按“倾斜的交易区间”处理：多头宽通道常在最近上涨腿的下部回调区域寻找顺势买入，空头宽通道镜像地在最近下跌腿的上部反弹区域寻找顺势卖出。课程总括时把最近一腿靠原趋势起点方向的约三分之二区域视为宽泛参与区；部分深回调案例又具体使用约 `60%–70%` 回撤，以及靠近腿起点的底部或顶部三分之一。两种口径的观察精度不同，不能合并成精确、互斥的价格带，也不能覆盖最新动量、结构 stop、目标空间与 Trader's Equation；若回调已经获得足够反向接受，必须先更新 market-cycle 和 Always In 判断。
