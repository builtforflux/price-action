...
同一机会可以有早入、确认后入场或回调入场等不同表达。出现多条完整表达时，分别比较当时的定性可能性、Entry、Stop、现实 Target、成本、期限和整仓风险：

| 比较结果 | 决定 |
| --- | --- |
| 能够作出有意义的取舍 | 只冻结当前真正准备执行的一条；其他表达不是隐藏订单 |
| 尚缺一项能够改变取舍的明确事实 | `WAIT`，并写出下一事件与期限 |
| 无法形成有意义的取舍，也没有明确下一事件 | `STAND_DOWN` |
...
- 仍在形成的 K 线不能按未知收盘、最终高低点或完整影线评价；
- Entry Basis 要求 close、follow-through 或回踩守住时，必须等待该事实完成；
- 盘中入场只依赖当时已经可观察且事前定义的条件。

H/L 使用同一条链：

```text
市场页识别测试次序与恢复边界
→ Opportunity 判断路径是否现实
→ 本页决定是否采用该边界
→ 价格越界后才形成 Chart Entry Bar
```

H2、Double Bottom、Reversal Bar 和 Signal Bar 可以描述同一事实的不同职责，但不能重复增加胜算。
...
可观察参照可以是区域边界、前一根 K 线高低点、突破点、已固定段的比例位置、趋势线或通道线。每个参照必须说明它在当前 Opportunity 中承担的职责，不按参照数量增加理由。

Limit 是否足以表达当前入场，按成交前仍缺少的事实判断：

| 成交前仍缺少的事实 | 可用表达 |
| --- | --- |
| 触及所选价格就可以承担风险 | 普通 Limit |
| 仍要求拒绝、守住、收盘或恢复 | 普通 Limit 不足；先等事实完成，再选择 Market / close、Stop 或后续回试 Limit |
...
- Stop entry 触发后仍要观察跟随、区域是否守住和分离是否建立；
- 入场停止单与 protective stop 职责不同；
- Limit touch / cross 不保证实际成交；
- 没有符合当前表达的 Entry 时不因 FOMO 追入；临时追价已经改变 Entry、Stop、目标与风险，必须重新判断。
...
- H2 Buy Stop 使用已完成 H2 Signal Bar 高点上方的最小价格单位；L2 镜像使用信号低点下方的 Sell Stop；
- Trigger 前在支撑或阻力区域用 Limit 成交，是同一 Opportunity 的提前表达，不得记成 H2 / L2 已经触发；
- Trigger 后等待回试再用 Limit，Trigger 是已经发生的入场依据，实际成交来自后续回试。
...
| 变化类型（可组合） | 处理 |
| --- | --- |
| 普通移动保护 | 只能向降低开放风险的方向进行 |
| 增加数量、放宽已经确认的 Stop、延长持有时间或改变原退出承诺 | 形成新的计划调整；重新判断，并确认调整后的整仓风险仍可接受 |
...
→ 可以形成把 Planned Stop 移到该结构之外的新计划

- 仅因价格盈利、经过个人成本价或害怕回吐，不足以产生新的价格保护位置；
- 只有 Broker 确认后，Active Protective Stop 才真正改变。
...
近似 breakeven 只有在以下任一条件成立时才可采用，并计入费用和滑点：

- 入场附近已经形成新的结构参照；
- 原计划明确把它作为资本、时间或执行行为边界。

其余边界：

- 个人成本价不能因此改写成市场支撑阻力；
- 被保护、主动退出或目标带出后，原市场 Opportunity 可能仍有效；
- 再次入场仍须形成当时完整的 Entry、Stop、Target、成本、期限与风险；
- 旧订单和旧数量不得自动恢复。
...
Scalp 与 Swing 只是不同的目标、Horizon 和风险实现。Scalp 目标近、成本占比高，通常要求更强确定性。Swing 目标远，需要容纳正常 pullback，并用更小 Size 承担更宽风险。

TBTL 是路径启发式，不是保证或机械最低根数。
...
- 未用风险余额只是整笔交易上限与当前计划损失的差；
- 它不是仓位，也不自动授权后续加仓；
- 调整时账户状况已经变化，则同时服从当时更严格的账户约束。
...
| --- | --- |
| 已成交部分 | 各部分真实 Entry |
| 尚未成交部分 | 计划允许的最坏 Entry |
| 多笔仓位或工作订单并存 | 各自 Stop 风险、仍可能成交订单的新增风险，以及相关头寸在同一不利行情中同时受损的风险 |

所有适用行都必须累计，不能只选择其中一项。
...
更低或更高的价格本身不是加仓理由。加仓订单按所需事实选择：

| 当前表达 | 订单选择 |
| --- | --- |
| 愿意用更好价格换取较少确认 | Limit |
| 需要新的响应或原方向恢复 | Stop entry |
| 条件不完整 | 不加仓 |
...
1. 以当前可执行价格计算当前账户开放风险，并与当时的账户及组合约束比较；
2. 以各层 Entry 到对应 Stop 计算相对入场的计划损失，并与整笔交易风险上限比较；
3. 计入所有仍可能成交的 Entry / Add 订单，不只计算新的一笔；
4. 多个订单可能同时成交时，按全部成交的最坏组合计算；只有已经确认可靠的互斥机制才能排除重叠。

两类整仓风险和订单最坏组合都可接受，才继续加仓；若 Add 只能靠放宽原 Stop、忽略已有风险或假定未经确认的订单互斥才能成立，取消 Add。
...
- Opportunity 满足以下任一资格：
  - 已激活且未失效；
  - 所选订单的触发能够完整实现尚缺的激活事实，并满足[2.2](#22-机会激活与交易触发)的全部条件；
...
- `TRADE` 只冻结当时的交易计划，不代表订单已提交、已经成交或保护已经在场；
- 激活前的 `TRADE` 只授权符合 2.2 的条件订单；
- 触发真正发生前，不得记录机会已激活或 Chart Entry Bar；
- Broker 确认成交前，不得记录 Actual Fill 或 Position；
- `WAIT` 始终不包含活动入场订单。
