...
Context Update：Keep / Transition / Reframe / Unclear
Long / Short：各自的 Already → Next 是否改变
Route：No Change / Update Opportunity / Rebuild Candidate / Update Position
```
...
| 接受与失败 | 边界外保持、回踩守住、快速收回或旧区域重新接受 | Current Move → Active Test.Stage | 哪个价格区被接受；原路径只是削弱还是已经失效？ |
...
Next：顺势 signal / breakout + follow-through；或穿越 EMA、反向跟随并获得接受
Expiry：首次测试完成、区域被反复穿越，或 Context 已重构
```
...
Next：follow-through、回踩守住；或分离关闭并重新接受旧区域
...
重开：Current Move.Continuity / Separation / Opponent → Active Test.Stage
双向：原方向是否新增独立跟随；反方是否关闭分离并建立自己的连续性？
Next：接受、breakout pullback 守住；或 failed breakout 所需的旧区域重新接受
...
Next：顺势恢复 / H1-H2 trigger；或回踩失败与旧区域接受
...
Next：区域外接受；或拒绝、反向 breakout 与 follow-through
Expiry：测试逻辑被接受/失败解决，或新 Context 取代原测试
```
...
Next：原方向恢复并再创新极值；或反方 breakout、跟随并破坏旧结构
Expiry：第四次有效推进、原趋势恢复，或反方获得接受并重构 Context
```
...
Next：突破 follow-through / hold；或快速收回并向另一侧测试
...
Next：旧极值突破并恢复原趋势；或测试失败、反向跟随并完成 MTR 路径
Expiry：旧趋势重建，或新方向结构获得接受
```
...
Next：目标外 follow-through / hold；或拒绝、反向 trigger 与跟随
Expiry：目标事件按既定口径完成，或原 horizon 到期
```
...
新增复合卡需同时满足：高频或高影响；容易漏掉；能明确指出应重开哪几步；能给出双向问题、Next 与 Expiry。只提供新名称、固定 Entry 或与现有卡相同路由的场景，归入已有卡的 Views。
