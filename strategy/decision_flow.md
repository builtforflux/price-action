...
1. **定位候选并建立各自 entry/stop/target**：按「基础状态 → 阶段/叠加层 → 结构进度 → 空间检查 → 时段覆盖」（与矩阵列顺序一致）在 [覆盖矩阵](coverage_matrix.md) 找到命中行。**G01（全部候选均无空间）与 R28（尾盘时间）不前置**——没有候选就不知道 target、stop 和持有期：
   - STRATEGY → 打开该行链接的策略页；
...
flowchart TD
    ...
    TR2 -->|"旧极值被重新接受并获跟进，或新持续紧密趋势腿重新建立控制"| W34["WATCH：MTR 证据链重置（R34）"]
...
- [覆盖矩阵](coverage_matrix.md)（行号 R01–R46、全局行 G01 与课程核验列）
- [策略目录入口](README.md)
