...
1. 每次读图先按「基础状态 → 阶段/叠加层 → 结构进度 → 空间检查 → 时段覆盖」的顺序（与矩阵列顺序一致）识别状态与候选，再由 `PREMISE_SELECT` 统一检查候选空间（G01）、硬 NO_TRADE 约束、至少两个相互补充且不重复计票的理由、opposing evidence / update condition、Trader's Equation 与等待条件，输出唯一叶子；
2. 每行只能落一种叶子：`STRATEGY`（可交易策略页）/ `WATCH`（候选观察或等待新证据）/ `NO_TRADE`（显式不交易）。`STRATEGY` 表示进入该策略页继续检查证据汇合与六要素（premise、触发、失效、stop、目标、管理），并据此建立 Trade Plan，不直接构成交易许可；
...
| 枚举 | 含义 | 处理 |
| --- | --- | --- |
| STRATEGY | 可交易策略页 | 进入该策略页继续检查独立理由、反方证据、更新条件与六要素，并据此建立 Trade Plan |
