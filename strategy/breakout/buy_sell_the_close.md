...
- 存在清楚旧边界，当前出现足够强的 breakout bar、连续强收盘或 follow-through；
- 新价格没有被快速收回，到现实目标仍有空间；
- 早期版只在首根足够强 breakout bar 收盘时成立；已经形成可识别回踩时，路由到突破回踩策略；
- 突破失去跟进并回到旧区域时，不继续使用 BTC/STC；状态转为 channel、trading range 或 climax / transition 时重新分类。
...
完整 breakout leg 外，必须按承担风险版本使用当时真实结构，不能借用另一版本更窄的 stop。

## 直接预期与目标

强 breakout 才常支持至少第二腿或 measured move；量度对象必须在当时可见并固定。

## 管理边界

强趋势中若选择按 scalp 很快退出，应事先承认回调可能不给出舒服的重新入场机会；只有入场前预写的 swing 或分仓版本才能保留 runner。关键收盘持续失望或阶段改变时停止机械 BTC，并按 premise 更新退出。
