...
回踩 swing 外，或足以证明旧区域重新接受的边界外。Stop 必须容许突破点附近的正常测试。

## 直接预期与目标

直接预期是第二腿或清楚结构的 measured move。回踩守住是额外接受证据，**不是突破获接受的必要步骤**。

## 管理边界

确认版 entry 更差，scalp 或 swing 取决于剩余目标空间；正常回踩允许，旧区域重新接受或跟进消失时按 premise 变化退出。两个确认版本分别使用承担风险时可见的证据，不能互借价格与概率。
