...
- 当前仍处于计划定义的 session opening 语境，opening-range 边界来源已经在观察前声明；
- 单次越界只构成 breakout attempt，必须等待边界外收盘、follow-through 或未被快速收回的接受证据；
- 尚未突破时路由到 trading-range / breakout-mode 逻辑；突破后形成回踩则可路由到突破回踩策略；
- 到现实 opening magnet、前高低或量度区域仍有足够时间和空间。
...
足以否定“边界外新价格获接受”的结构外——通常是突破 leg 结构外，或价格重新进入开盘范围并获反向跟进的位置外。

## 直接预期与目标

直接预期是开盘突破延续；开盘范围高度投射或前高前低等 magnet 是目标候选。时段结束前剩余时间不足会压缩远端 swing 目标的可达性。

## 管理边界

Scalp 或 swing 取决于突破强度与剩余时间；正常回踩允许，重新进入开盘范围并获接受时按 premise 变化退出。盘中不能用最终 day type 回填原计划。
