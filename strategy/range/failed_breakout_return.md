...
失败极值外；只有再次越过该位置足以否定 fade premise 时，它才是完整 stop 锚点。

## 直接预期与目标

直接预期是回到区间内部价值区域。若区间另一侧随后发生成功突破，区间高度 measured move 属于新的 breakout 命题，不再沿用本页目标。

## 管理边界

按 scalp 管理区间内部目标；再次越界获接受时按 premise 变化退出；时段尾段剩余时间不足时，不继续加仓等待均值回归。突破失败后也不能机械反手，必须等待新的独立 Setup。
