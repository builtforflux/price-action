...
## 案例验收（价格回放验收前不批量复制；完整覆盖表见[回放记录](buy_sell_the_close_playback.md)）

| 案例 | 预期结果 | 状态 |
| --- | --- | --- |
| 正例：早期版 | 首根强 bar 收盘、无独立 follow-through、五条严格口径全满足 → 早期版 + 完整 leg stop + 管理版本选择 | 构造走查通过 |
| 正例：确认版 | 连续强收盘或独立 follow-through 已出现 → 确认版，stop/target 按确认时重算 | 构造走查通过 |
| 近似反例 | 突破 K 很大但收盘 / 重叠质量不足（严格口径第 3–4 条不满足）→ WATCH | 构造走查通过 |
| 管理例 | 正常回调进入 channel → 不退出，按原计划管理；新入场转趋势/pullback 策略 | 构造走查通过 |
| 失效例 | 旧区域被重新接受并获反向跟进 → premise 失效，主动退出 | 构造走查通过 |
| 过期例 | 尚未入场已出现回踩 → 本卡不可用，转 breakout-pullback 重新评估 | 构造走查通过 |
| 管理例：Scalp | 没有可行 swing 计划（结构不清楚或 swing 方程不成立）、近端 magnet 的 scalp 方程成立 → Scalp + 最近 magnet target | 构造走查通过 |
| 管理例：Swing | 清楚 swing 结构且近端 magnet 不构成实质障碍 → Swing + 结构目标 | 构造走查通过 |
| 管理例：预写分仓 | 近端 magnet 显著影响远端概率、两套方程均成立且数量已拆分 → 预写分仓 | 构造走查通过 |
| 管理例：swing 方程被否定 | 有 swing 结构但近端障碍使 swing 方程不成立、scalp 方程成立 → Scalp（不被错误排除） | 构造走查通过 |
| 削弱例：momentum 削弱 | 完整削弱组合 → 允许预写分支减仓，不触发全部退出 | 构造走查通过 |
| 正常波动例：无跟进反向 signal | 单个普通反向 signal 无跟进：不单独构成 premise 失效；满足完整削弱组合才按削弱处理 | 构造走查通过 |
| 失效例：全部退出 | 强反向突破 + 跟进 → premise 失效，**全部退出而非只减仓** | 构造走查通过 |
| 失效例：异常强单根反向 K | 单根否定接受结构 → premise 失效，全部退出（不要求跟进） | 构造走查通过 |
