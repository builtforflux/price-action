...
> **状态：Strategy / Application（Decision Protocol）（试点验收通过，见回放记录）**
>
...
- **Premise invalidation（唯一定义，任一成立即触发）**：
  1. 价格明确回到旧区域并获得反向跟进；
  2. 在预先声明的管理周期上出现足以否定原接受结构的**强反向动量**，满足任一：
     a. 一根异常强反向趋势 K 线本身已经否定原接受结构，不要求后续跟进；
     b. 强反向突破并获得跟进；
  ...
  以下只作为第 1–3 条的回放校准说明（不引入定义之外的第四条规则），图形边界在回放中校准（不设机械参数）：

  - 单根普通反向 K 线，或只有强外观但未获得跟进、未使 Always In 翻转、未否定原接受结构 → **不自动退出**；
  - 异常强反向趋势 K 线本身否定原接受结构（第 2a 条）→ **Premise invalidation，全部退出**（core 三层退出保护的强反向动量层，不要求后续跟进）；
  - 强反向突破 + 跟进（第 2b 条）→ **Premise invalidation，全部退出**；
  - 正常 pullback 进入 channel → **不退出**。
- **Active exit 预写**：
...
| 案例 | 预期结果 | 状态 |
| --- | --- | --- |
| 正例：早期版 | 首根强 bar 收盘、无独立 follow-through、五条严格口径全满足 → 早期版 + 完整 leg stop + 管理版本选择 | 通过（见回放记录） |
| 正例：确认版 | 连续强收盘或独立 follow-through 已出现 → 确认版，stop/target 按确认时重算 | 通过 |
| 近似反例 | 突破 K 很大但收盘 / 重叠质量不足（严格口径第 3–4 条不满足）→ WATCH | 通过 |
| 管理例 | 正常回调进入 channel → 不退出，按原计划管理；新入场转趋势/pullback 策略 | 通过 |
| 失效例 | 旧区域被重新接受并获反向跟进 → premise 失效，主动退出 | 通过 |
| 过期例 | 尚未入场已出现回踩 → 本卡不可用，转 breakout-pullback 重新评估 | 通过 |
| 管理例：Scalp | 没有可行 swing 计划（结构不清楚或 swing 方程不成立）、近端 magnet 的 scalp 方程成立 → Scalp + 最近 magnet target | 通过 |
| 管理例：Swing | 清楚 swing 结构且近端 magnet 不构成实质障碍 → Swing + 结构目标 | 通过 |
| 管理例：预写分仓 | 近端 magnet 显著影响远端概率、两套方程均成立且数量已拆分 → 预写分仓 | 通过 |
| 管理例：swing 方程被否定 | 有 swing 结构但近端障碍使 swing 方程不成立、scalp 方程成立 → Scalp（不被错误排除） | 通过 |
| 削弱例：momentum 削弱 | 顺势 K 线缩小 / 跟进丢失 + 反方 K 线增加但未形成强反向突破 + 旧区域尚未重新接受（完整削弱组合）→ 允许预写分支减仓，不触发全部退出 | 通过 |
| 正常波动例：无跟进反向 signal | 单个普通反向 signal 触发但无跟进：不单独构成 premise 失效；若同时满足完整削弱组合则按削弱处理，否则视为正常波动 | 通过 |
| 失效例：全部退出 | 强反向突破 + 跟进 → premise 失效，**全部退出而非只减仓** | 通过 |
| 失效例：异常强单根反向 K | 即使尚无后续跟进，但已否定接受结构或使 Always In 明确翻转 → premise 失效，全部退出 | 通过 |
