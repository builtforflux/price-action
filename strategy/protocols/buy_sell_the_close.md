...
- **Premise invalidation（唯一定义，任一成立即触发）**：
  ...
  第 2、3 条的图形边界在回放中校准（不设机械参数），至少区分：

  - 单根普通反向 K 线，或只有强外观但未获得跟进、未使 Always In 翻转、未否定原接受结构 → **不自动退出**；
  - 一根异常强反向趋势 K 线，本身已经在预先声明的管理周期上否定原接受结构或使 Always In 明确翻转 → **Premise invalidation，全部退出**（core 三层退出保护的强反向动量层，不要求后续跟进）；
  - 强反向突破 + 跟进 → **Premise invalidation，全部退出**；
