...
- **Active exit 预写**：
  ...
  - 要求主动退出的反向突破与跟进：即上方 **Premise 失效**（强反向突破 + 跟进，或旧区域被重新接受）——在结构 stop 前主动退出，此处不重复定义；
  - Always In 翻转采用的管理周期：以本仓 entry 与 stop 所在的交易周期为准，不切换到更低周期寻找退出理由；
  - scalp / swing 退出边界（按可观察事件区分）：
    - 单根普通反向 K 线：正常波动，不退出；
    - 反向 signal 尚未触发：正常波动，不退出；
    - 强反向突破并获跟进，或旧区域被重新接受：**premise 失效 → 主动退出**（同上方 Premise 失效条，不重复定义）；
  ...
  - 部分退出：仅当入场前选择「预写分仓」版本时允许；触发只允许**预写 target 或 premise 削弱证据**；**premise 失效触发全部退出**。保留 runner 仅限入场前已写明其独立管理边界（若剩余仓位依赖另一 premise，它必须从入场前就是独立预写的 runner 计划，不能在失效后临时改名）。每份的数量、target 与触发事件必须入场前写明。
...
| 削弱例：momentum 削弱 | 顺势 K 线缩小 / 跟进丢失，但旧区域尚未重新接受 → 允许预写分支减仓，不触发全部退出 | 待回放 |
| 正常波动例：无跟进反向 signal | 单根普通反向 signal 触发后没有跟进 → 正常波动，不误判为 premise 失效 | 待回放 |
