...
来源材料常用的历史术语，最低含义是带有 Context、可供考虑入场的 pattern。Setup 不等于完整风险计划；若要进入运行方法，仍须能还原为可观察条件、目标事件、参与条件、失效、概率边界和风险交换。Reference 不要求系统维护 Setup 家族或独立路由。
...
Pressure 概括一侧的方向表达与持续性；比较 buying pressure 与 selling pressure 时，还要限定同一周期、运动、区域和问题。当前方法共享实体、影线、收盘、分离与回调事实，分别解释本侧进展及相对对手表现；双方同强与双方同弱都可能相对平衡，却有不同波动与后续预期。Control 表示持续能力足以组织正常回调、预期和路径。一侧减弱不等于另一侧已经增强。Always In 是清晰方向判断的来源摘要，不表示必须持仓，也不替代目标和交易方案。

### Gap / Separation

Gap / Separation 比较两个相关价格对象之间的分离或重新重叠程度；正 gap 保留未充分双边交易的空间，negative / overlap gap 表示相关锚点已经轻微重叠。传统 gap、body gap、opening gap、micro gap 与突破点—回调 gap 使用不同参照对象；任何机械标注都必须写清参照。Measuring / exhaustion 是背景和后续共同形成的解释，只有后续路径才能确认，不是另一种无需锚点的几何关系。
...
- 这是需要保留的结果不确定性；当前方法按“先后未知”记录，补充可靠数据后再确认。
